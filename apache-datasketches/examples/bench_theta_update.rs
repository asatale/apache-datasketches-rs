//! Throughput harness for `ThetaSketch::update_u64`.
//!
//! Run with (release matters -- a debug build measures nothing useful):
//!   cargo run --release --example bench_theta_update --features theta
//!   cargo run --release --example bench_theta_update --features theta -- 100000000
//!   cargo run --release --example bench_theta_update --features theta -- --ladder
//!
...
//!
//! That harness takes the same arguments and prints the same format. Keep the
//! two in step -- in particular the output format, the median definition and
//! the string-key format; the mechanism lives in its `bench_common.h`.
//!
//! Fixed parameters so runs are comparable: `lg_k = 12`, defaults elsewhere.
//! Keep them in sync with `benches/cpp_reference/theta_update.cc`; both print
//! the estimate, and the estimates must match.
//!
...
use apache_datasketches::theta::{
    jaccard_similarity, CompactThetaSketch, ThetaIntersection, ThetaSketch, ThetaSketchBuilder,
    ThetaUnionBuilder,
};
...
fn bench_serde(items: u64, reps: usize) {
    ...
    let deserialize =
        || CompactThetaSketch::deserialize(&reference).expect("the bytes came from serialize");
    let mut passes = Vec::with_capacity(reps);
    for _ in 0..reps {
        let start = Instant::now();
        let mut total = 0.0;
...
fn build_operands(items: u64) -> (CompactThetaSketch, CompactThetaSketch) {
    let mut a = build();
    for key in 0..items {
        a.update_u64(key);
    }
...
fn bench_union(items: u64, reps: usize) {
    ...
    for _ in 0..reps {
        ...
        for _ in 0..OP_CALLS {
            let mut union = ThetaUnionBuilder::new()
                .lg_k(LG_K)
                .build()
                .expect("fixed valid parameters were rejected");
            union.update(&a);
...
/// `jaccard_similarity` is a pure function of its two operands -- no
/// accumulator to rebuild, unlike union and intersection.
fn bench_jaccard(items: u64, reps: usize) {
    ...
    for _ in 0..reps {
        let start = Instant::now();
        let mut total = 0.0;
        let mut bounds = jaccard_similarity(&a, &b);
        for _ in 0..OP_CALLS {
            bounds = jaccard_similarity(&a, &b);
